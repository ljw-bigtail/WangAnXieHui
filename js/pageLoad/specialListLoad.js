...
$(function() {
    ...
    getMenu(function(menu) {
        ...
        if(parentMenuTitle == 'config'||parentMenuTitle==''&&grandParentMenuTitle==""){
            $(".breadLink").html('当前位置：<a href="'+menu[0].enName+'.html">首页</a> > <span>'+menuTitle+'</span>');
    ...
    getPubList(contentListOpt,function(list) {
        if(list.currentTotal==0){
            $(".changePage").hide();
            $(".specialList").html("<div class='noData'>无数据</div>");
        }else{
            $(".changePage").show();
            $(".specialList").html("");
            var num=Math.ceil(list.currentTotal/contentListOpt.pageSize);
            initList(list,$(".specialList"),$(".changePage"),num);
        }
...
function initList(list,$dom1,$dom2,num){
    ...
    for(var a in list.list){
        var $content=$("<li><div class='picPart'><a target='_blank' href='"+ list.list[a].mainUrl+"'><img src='"+list.list[a].mainPic+"'></div><div class='mesPart'><a href='./content.html?id=" + list.list[a].id +"'>"+list.list[a].title+"</a><span>"+list.list[a].conDate.split(/T/g)[0]+"</span><p>"+list.list[a].summary+"</p></div></li>");
        $content.appendTo($dom1);
    }

    /*list.list.forEach(function(item){
        var $content=$("<li><div class='picPart'><a href='"+ item.mainUrl+"'><img src='"+item.mainPic+"'></div><div class='mesPart'><a href='./content.html?id=" + item.id +"'>"+item.title+"</a><span>"+item.conDate.split(/T/g)[0]+"</span><p>"+item.summary+"</p></div></li>");
        $content.appendTo($dom1);
    });*/
...
function initSidebar(content, $con){
    if(content.menuTitle!="config"&&content.menuTitle!=''){
        console.log("111")
